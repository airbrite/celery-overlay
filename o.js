(function(root) {
  ...
  // ---
  // Prefill querystring
  // Fix google analytics self referral
  // Add google analytics ecommerce events

  // Don't load again if Celery has already loaded
  if (root.Celery) { return; }

  // Don't use global jQuery (it's included)
  // Provide a local context for `$`
  var $;

  // Export Celery to window/root
  var Celery = root.Celery = {};
  ...
  // Whitelisted SLUGS that are automatically V2
  Celery.slugsToMigrate = ["a7bdcb26-5124-4bd4-9c70-c62e5910d706", "thebradley-mustard", "thebradley-stainlesssteel", "thebradley-silverblue", "thebradley-olivegreen"];

  // Store all Celery elements that should launch an overlay
  // Each one has a corresponding index to provide uniqueness
  Celery.index = 0;
  Celery.els = [];

  // Load an external script asynchronously and callback when ready
  // This is a DEBUG only method, not used in production
  Celery.loadScript = function(url, callback) {
  ...
  // Cross browser compliant add event listener
  Celery.addEvent = function(target, event, fn) {
  ...
  // Cross browser compliant remove event listener
  Celery.removeEvent = function(target, event, fn) {
  ...
  Celery.preventDefault = function(e) {
    if (e) {
  ...
  Celery.stopPropagation = function(e) {
    if (e) {
      // no bubble
  ...
  Celery.preventDefaultStopPropagation = function(e) {
    this.preventDefault(e);
    this.stopPropagation(e);
  };

  // Inject CSS styles at the end of HEAD
  Celery.appendStyles = function(styles) {
  ...
  Celery.detectCSSFeature = function(featurename) {
    var feature = false,
  ...
  // An attempt to get the google analytics linker URL and appending it to the celery shop URL query string
  Celery.getLinkerUrl = function(item, callback) {
    this.getLinkerUrlOld(item, function(urlOld) {
      this.getLinkerUrlNew(item, function(urlNew) {
        return callback(urlNew);
      }.bind(this));
    }.bind(this));
  };

  // Append the google analytics linker url from the parent site if present
  // This is for the OLD google analytics
  // https://developers.google.com/analytics/devguides/collection/analyticsjs/cross-domain
  Celery.getLinkerUrlOld = function(item, callback) {
    var pageTracker, linkerUrl;

    // If _gat is already loaded, this is a sync call
    if (root._gat) {
      pageTracker = root._gat._getTrackerByName();
      if (!pageTracker) {
        return callback(item.src);
      }
      linkerUrl = pageTracker._getLinkerUrl(item.src);
      if (!linkerUrl) {
        return callback(item.src);
      }
      item.src = linkerUrl;
    ...
    // Make sure _gaq exists and it has an account set
    // This is an async call
    if (root._gaq && root._gaq.length > 0) {
      return root._gaq.push(function() {
        pageTracker = root._gat._getTrackerByName(); // Gets the default tracker.
        if (!pageTracker) {
          return callback(item.src);
        }
        linkerUrl = pageTracker._getLinkerUrl(item.src);
  ...
  // Append the google analytics linker url from the parent site if present
  // This is for the NEW unified google analytics
  Celery.getLinkerUrlNew = function(item, callback) {
    if (root.ga) {
      return root.ga(function(tracker) {
        if (!tracker) {
          return callback(item.src);
        }
        ...
        var linkerUrl = linker.decorate(item.src);
        if (!linkerUrl) {
          return callback(item.src);
        }
        item.src = linkerUrl;
  ...
  Celery.initialize = function() {
    ...
    // Our shop page sends postMessages when inside an iframe
    Celery.addEvent(window, "message", Celery.receiveMessage.bind(this));

    // Start bindElements every 500ms
    this.bindElements();
    setInterval(this.bindElements.bind(this), 500);
  };

  // Whitelist slugs to v2
  // This is a temporary method during the v1 -> v2 migration
  Celery.getVersion = function(options) {
    var version = options.version;

    if ($.inArray(options.slug, this.slugsToMigrate) >= 0) {
      version = "v2";
    }
  ...
  // Find all elements with attribute `data-celery` and click bindings
  Celery.bindElements = function() {
    this.$el.find('[data-celery]').each(function(index, el) {
      var $el = $(el);

      // Existing element
      // Unbind click and remove from els
      if ($el.attr('data-celery-index')) {
        for (var i = this.els.length - 1; i >= 0; i --) {
          if (this.els[i].index === $el.attr('data-celery-index')) {
            if (this.els[i].type === 'overlay') {
              $el.off('click', this.els[i].fn);
            }
            this.els.splice(i, 1);
          }
        }
      } else {
        // Add unique index incrementing to el
        $el.attr('data-celery-index', ++this.index);
      }

      // Read all `data-celery-*` attributes
      var options = {
        index: $el.attr('data-celery-index'),
        slug: $el.attr('data-celery'), // in v2 this is _id
        type: $el.attr('data-celery-type') || 'overlay',
        env: $el.attr('data-celery-env') || 'production',
        version: $el.attr('data-celery-version') || 'v1'
      };

      // Execute version override whitelist
      options.version = this.getVersion(options);

      // Remove `data-celery` attribute to avoid reparsing
      $el.removeAttr('data-celery');

      // Add item to list of Celery shops
      var item = {
        el: el,
        $el: $el,
        host: this.getHost(options),
        src: this.getUrl(options),
        index: options.index,
        slug: options.slug,
        type: options.type
      };
      ...
      if (item.type === 'embed') {
        // Immediately launch the embed if using embedded iframe type
        this.launchEmbed(item);
      } else {
        // Attach click event for overlay type
        item.fn = this.launchOverlay.bind(this, item);
        $el.on('click', item.fn);
      }
    }.bind(this));
  };

  // Compute the host based on env and version
  Celery.getHost = function(options) {
    var host;

    if (options.version === 'v2') {
      switch (options.env) {
        case 'development':
          host = "http://dev.trycelery.com:5555";
        break;
        case 'sandbox':
          host = "https://shop-sandbox.trycelery.com";
        break;
        case 'staging':
          host = "https://shop-staging.trycelery.com";
        break;
        case 'production':
          host = "https://shop.trycelery.com";
        break;
        default:
          host = "https://shop.trycelery.com";
        break;
      ...
      switch (options.env) {
        case 'development':
          host = "http://dev.trycelery.com:3333";
        break;
        case 'sandbox':
          host = "https://sandbox.trycelery.com";
        break;
        case 'staging':
          host = "https://staging.trycelery.com";
        break;
        case 'production':
          host = "https://www.trycelery.com";
        break;
        default:
          host = "https://www.trycelery.com";
        break;
  ...
  // Compute the iframe src/url based on version and slug
  Celery.getUrl = function(options, callback) {
    // Host
    var url = this.getHost(options);

    // Slug
    if (options.version === 'v2') {
      if (options.type === 'embed') {
        url += "/embed/" + options.slug;
      } else {
        url += "/overlay/" + options.slug;
      }
    } else {
      url += "/shop/" + options.slug;
    }
  ...
  // Build the query string that will be appended to the iframe src/url
  Celery.getQueryString = function(options) {
    var params = {
      type: options.type,
      index: options.index,
    ...
    if (this.isMobile && options.type === 'overlay') {
      delete params.type;
    }
  ...
  // Build an iframe that will contain the embed
  Celery.buildEmbed = function(src) {
    var $embed = $('<iframe marginheight="0" marginwidth="0" frameborder="0"></iframe>');
    $embed.attr({
      src: src,
      border: '0'
    });
  ...
  // Show the embed
  Celery.launchEmbed = function(item) {
    // Build embed
    this.getLinkerUrl(item, function(url) {
      item.$el.html(this.buildEmbed(url));
  ...
  // When using an embed, the height needs to be set by the iframe document
  // On the iframe side we post messages with the update height so that we can update it here
  Celery.updateEmbedHeight = function(src, height) {
    var $embedEl = $('iframe[src="' + src + '"]');
    $embedEl.css("height", height);
  };
  ...
  // Build an iframe that will contain the overlay
  Celery.buildOverlay = function(src) {
    var $overlay = $('<iframe marginheight="0" marginwidth="0" frameborder="0"></iframe>');
    $overlay.attr({
      src: src,
      border: '0'
    });
  ...
  // Build a div that serves as a transparent backdrop
  Celery.buildBackdrop = function() {
    var $backdrop = $('<div class="celery-backdrop celery-animated celery-backdrop-fade-in"></div>');

    return $backdrop;
  };

  // Build a div that serves as the loading spinner
  Celery.buildLoader = function() {
    var $loader = $('<div class="celery-spinner"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>');
    return $loader;
  };

  // Show the overlay
  Celery.launchOverlay = function(item, e) {
    this.preventDefaultStopPropagation(e);

    if (this.isMobile) {
      return (window.location.href = item.src);
    }

    // Current item, one at a time
    // If an item is already showing, no-op
    if (this.currentItem) {
      return false;
    }
    ...
    // Disable overflow
    this.$el.addClass('celery-open');

    // Build backdrop and loader
    this.$backdropEl = this.buildBackdrop();
    this.$loaderEl = this.buildLoader();
    this.$el.prepend(this.$loaderEl);
    this.$el.prepend(this.$backdropEl);

    // Build Overlay
    this.getLinkerUrl(item, function(url) {
      // console.log("overlay linker url: " + url);
      this.$overlayEl = this.buildOverlay(url);
      this.$overlayEl.css('opacity', '0');
      this.$el.prepend(this.$overlayEl);

      // 2014/03/26 - THE BELOW CODE IS NOT FUNCTIONAL, PLEASE IGNORE
      // ---
      // We don't use onLoad to show the overlay, instead we use window.postMessage
      ...
      // $(document).ready, however, fires when the DOM tree is complete and can be manipulated.
      this.$overlayEl.on("load", function() {
        // TODO IE has issues with this
        // If the frame has loaded but hasn't been shown, something bad happened
        // if (!this.isShowing) {
        //   alert("We're having trouble loading the Celery shop. Please check to make sure your product is published and your embed code is entered correctly.");
  ...
  Celery.hide = function() {
    // Enable overflow
    this.$el.removeClass('celery-open');
    ...
    if (!this.detectCSSFeature("animation")) {
      this.$backdropEl.remove();
    } else {
      this.$backdropEl.addClass('celery-backdrop-fade-out');
      this.$backdropEl.one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        this.$backdropEl.remove();
      }.bind(this));
    }
  ...
  Celery.ready = function() {
    // GA pageview tracking
    if (root._gaq) {
      root._gaq.push(['_trackPageview', '/celery/' + this.currentItem.slug]);
    }
  ...
  Celery.placed = function(order) {
    // GA pageview tracking
    if (root._gaq) {
      root._gaq.push(['_trackPageview', '/celery/' + this.currentItem.slug + '/placed']);
    }
  ...
  // The order cancelled
  Celery.cancelled = function(order) {
    // GA pageview tracking
    if (root._gaq) {
      root._gaq.push(['_trackPageview', '/celery/' + this.currentItem.slug + '/cancelled']);
    }
  ...
  // The order failed
  Celery.failed = function(order) {
    // GA pageview tracking
    if (root._gaq) {
      root._gaq.push(['_trackPageview', '/celery/' + this.currentItem.slug + '/failed']);
    }
  ...
  // Cross-iframe message posting
  // ---
  
  // Post a message
  Celery.postMessage = function(data) {
    var jsonData = JSON.stringify(data);
    if (window.top) {
      window.top.postMessage(jsonData, "*");
  ...
  // Receive a message
  Celery.receiveMessage = function(e) {
    try {
      ...
      if (message.action) {
        switch (message.action) {
          ...
          break;
          case 'keyup':
            // Backwards compat for v1
            if (message.selector === 27) {
              this.hide();
            }
          break;
          case 'height':
            ...
            var src = message.src;
            this.updateEmbedHeight(src, height);
          break;
          case 'confirmation':
      ...
      //   console.log("message parse error: ", e.data);
      //   console.log("message parse exception: ", exception);
      // }
    }
  };
  ...
  // NOTE
  // Bug with almond.js in global scope causes anonymous defines to fail, so in this context let's overwrite define
  // https://github.com/jrburke/almond/issues/25
  var define;
  ...
  $(document).ready(Celery.initialize.call(Celery));
})(this);